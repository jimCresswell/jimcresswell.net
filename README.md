...
- **bash 5.2 or later** — every bash script in the repository opens with a guard that stops an older bash with install advice, and the two secrets hooks block until it is met (macOS ships 3.2: `brew install bash` and put it first on `PATH`; Debian and Ubuntu: `sudo apt-get install bash`). The shellcheck gate holds the floor once and requires the guard. Git hooks run under `sh` and carry no floor.
- **jq** — required by quality gates: the secrets hooks read their input with jq when it is installed and without it otherwise, and their smoke tests prove both paths (`brew install jq` on macOS if `/usr/bin/jq` is absent, `sudo apt-get install jq` on Debian and Ubuntu, see [jq downloads](https://jqlang.org/download/) for other platforms)
