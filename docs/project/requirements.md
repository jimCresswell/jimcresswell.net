...
## REQ-05: SEO and Discoverability

The site must be discoverable by search engines and present well when shared.

- All pages have appropriate `<title>` and `<meta description>` tags.
- Open Graph metadata is present for social sharing.
- Schema.org structured data (JSON-LD) on CV pages.
