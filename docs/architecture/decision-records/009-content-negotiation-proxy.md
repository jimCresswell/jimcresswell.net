...
2026-02-15 (amended 2026-02-23: self-fetch routing; 2026-08-12: preserve
upstream error status and restrict negotiation to explicit editorial documents)
...
5. **Representation-independent status** — Negotiating Markdown must not turn a missing page into a server error. If the HTML self-fetch returns a 4xx or 5xx response, the Markdown handler must preserve that status.
6. **Explicit negotiable documents** — Only the home and canonical CV editorial documents negotiate alternate representations. Native subroutes and missing routes retain their own status and content type.
...
Those conditions apply only when the resolved document path is `/` or `/cv`
(including the canonical trailing-slash form). Other paths pass through to
normal Next.js routing. This keeps `/cv/pdf` and `/cv/pdf/unavailable` native
and ensures retired or unknown slugs remain 404 for every requested media type.
...
This means each supported editorial document can be accessed as Markdown using either pattern — whichever the visitor finds more natural.

The runtime reports a failed HTML self-fetch as an error containing the
upstream HTTP status. The Route Handler preserves 4xx and 5xx statuses from
that error instead of flattening them to 500. A retired or unknown route
therefore remains 404 through both `Accept: text/markdown` and `.md` aliases.

The handler is a public HTTP route even though the proxy is its normal caller.
It therefore validates its caller-controlled path header or query parameter
against the same editorial-document allowlist as the proxy before starting a
self-fetch. Direct calls cannot use the deployment-protection bypass to fetch
native, retired, or unknown routes.

### Cache correctness

The home and canonical CV URLs select their representation from the `Accept`
request header. The Markdown and graph handlers include `Vary: Accept`
alongside Next.js's framework-provided fields.

Next.js 16 owns and replaces the rendered App Router HTML `Vary` field with its
React Server Component fields. The deployment therefore also relies on
[Vercel's documented default cache key](https://vercel.com/docs/caching/cdn-cache#vary-header),
which includes `Accept` and `Accept-Encoding` without requiring them in
`Vary`. Cloudflare currently treats these HTML routes as dynamic rather than
maintaining a second shared cache. If either hosting cache contract changes,
alternate-representation caching must be re-proved before enabling it.
...
Requesting `/` or `/cv` with `Accept: application/ld+json` returns the full Schema.org knowledge graph (see ADR-010 for the rationale behind "full graph, not per-page subgraph"). The proxy simply rewrites supported editorial documents to `/api/graph`, which serves the same graph that appears as `<script type="application/ld+json">` on the CV page. Other routes retain their native response.
