...
The Engraph uses `@engraph/logger` for structured logging with the shared observability foundation:
...
### Main Entry Point: `@engraph/logger`
...
### Node Entry Point: `@engraph/logger/node`
...
import { createNodeFileSink } from '@engraph/logger/node';
...
├─ YES → Use @engraph/logger (main entry)
└─ NO → Does it need file logging?
    ├─ YES → Use @engraph/logger/node
    └─ NO → Use @engraph/logger (main entry)
```
...
```typescript
import { normalizeError } from '@engraph/logger';
...
import { normalizeError, startTimer } from '@engraph/logger/node';
...
```typescript
import { startTimer } from '@engraph/logger';
...
```typescript
import { enrichError, normalizeError, type ErrorContext, startTimer } from '@engraph/logger';
...
import type { Logger } from '@engraph/logger';
...
import { createNodeStdoutSink } from '@engraph/logger/node'; // ❌ Breaks in browser!

// ✅ Use main entry for browser-compatible code
import { UnifiedLogger } from '@engraph/logger'; // ✅ Works everywhere
```
...
import { createNodeStdoutSink, createNodeFileSink } from '@engraph/logger/node';
```
