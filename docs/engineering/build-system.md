...
| **pre-push**   | `pnpm check`, then the site's end-to-end suite (`pnpm --filter @jimcresswell/www test:e2e`), each holding a host gate slot for its run (`pnpm agent-tools:gate-slot run pnpm ...`): at most two full gates on the host at once and one in a working tree, so a push may wait, naming the gates it waits for (`pnpm agent-tools:gate-slot status` lists them); a step past thirty minutes is stopped and fails. |
