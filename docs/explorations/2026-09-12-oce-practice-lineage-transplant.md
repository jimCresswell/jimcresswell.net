...
| Gemini                     | `GEMINI.md` (carried with item 4). The pin's 20 `.gemini/commands/review-*.toml` are the shape the adapter generator emits as its fourth platform, not an import; the pin's settings and product commands are not carried                                                                               | Carried (round 8; method by owner word, round 16: generated like every other adapter). The per-role commands land with lane B's generator and lane C's extension of it, neither yet opened |
...
## Transplant closure — proposed 2026-09-13 (afternoon), awaiting the owner's word

The owner: the transplant "is a bounded operation and I expect the transplant to finish, and
after that we are moving on to editorial work". The transplant is finished when this
repository has a working and full Practice (the owner's definition of 2026-09-13) and no longer
carries the source's residue except in the record. Everything else the programme has grown —
the installable entity, the transplant instruments as bins, the language separation — is
Practice evolution and moves to the Practice stream as post-transplant work. Every exit
condition below names the check that proves it, never a list a seat must remember
(`compute-dont-hope`). Ratify as one list; decline by number.

1. **Activate the installed instruments** (`practice-completion` todo 2): `practice-substrate`
   (its `pnpm -s` cured to `--silent`), `validate-protocol-wire-contract`, and the agent-tools
   smoke suite as legs of `check` or a named gate. Proof: the legs, CI parity, green.
2. **Completeness by function** (todo 3, bounded): the four generic rules re-triaged under
   "bring unless product"; the operational registers declared per the substrate manifest;
   mutation testing's absence stated (already in `testing-strategy.md`); the Gemini projection
   per the ruling; the definition report's nine rows read "present" with a cited proof each.
3. **Lineage residue leaves the live surfaces**: the two product files, five package
   manifests and fourteen test fixtures the plan names under §Oak residue in tooling; the
   `accept-md.config.js` hand-authored JavaScript; `turbo.json`'s unnamed `.next/` outputs; the
   consumed incoming bundle; the five retirement candidates among the agent-tools scripts
   decided by the owner. Proof: a lineage-name leak check over live surfaces (the transplant
   antigen scan landed as a validator, CV content excluded by scope) is green.
4. **`.agent-original/` deleted** after the owner rules on a presented loss-scan list (255
   files; every file dispositioned kept-elsewhere, superseded, or dropped). Proof: the
   directory gone, the dispositions in the plan of record.
5. **The last hand-kept copies derived**: the rules index and the Cursor rule triggers from
   frontmatter the canonical rules carry, generated by `portability:fix` and checked by
   `portability:check`; the sub-agent adapter descriptions from the templates. Proof: the
   generator's test and a byte-equal regeneration.
...
7. **The 57-lesson synthesis** over the archived napkin, one seat synthesising onto the
   graduations register, the owner reviewing before anything graduates (PRACTICE-2; the
   ordering note: after item 4). Proof: the register's entries and the owner's review.
8. **PR #53 ready and merged**, the site building from `main` on Vercel. Proof: merged.

Moved out of the transplant to the Practice stream (post-transplant): `practice-completion`
todo 5 and acceptance criterion 5 (the installable entity and the lineage-side proposals), todo
6's transplant instruments beyond the generators in item 5 (classify, digest as bins), and the
`practice-language-separation` node (sketch, not urgent). Applied to the node on the owner's
word.

Estimate, for the owner's planning only: items 1, 2, 5 and 6 are one working session each; item
