...
/**
 * Write the hook's one answer and let the process end on its own.
 *
 * The exit code is set, never forced with `process.exit`: stdout here is a pipe or
 * socket the harness reads, and a write to it that does not fit its buffer completes
 * asynchronously, so exiting straight after the write would cut the answer short.
 */
