...
- **Never work around checks** — If a variable is unused, figure out why and fix it. Always fix the
  root cause.
- **Quality gates** — Run ALL blocking gates after changes. `pnpm check` runs
  the full sequence with auto-fix where appropriate; `pnpm check:ci` runs it
  read-only. The blocking gates, in order:
  1. `pnpm format:fix` / `pnpm format` — Prettier
  2. `pnpm markdownlint:fix` / `pnpm markdownlint:check` — Markdown linting for authored docs
  3. `pnpm lint:fix` / `pnpm lint` — ESLint
  ...
  7. `pnpm secrets:scan` — secrets in git history
  8. `pnpm vital-surfaces:check` — vital Practice surface contract
  9. `pnpm portability:check` — thin-wrapper and local surface-contract validation
  10. `pnpm subagents:check` — reviewer wrapper and registry validation
  ...
  `pnpm fitness-vocabulary:check`. Fitness validators stay informational
  during this adoption and are promoted deliberately in a later reconciliation
  pass.
