...
| `manifest.webmanifest`        | Browser/install surfaces                                               | Identity text derives from `person`, so name and description stay aligned with the graph                                                        | `app/manifest.integration.test.ts`, `e2e/behaviour/manifest.e2e-api.test.ts`, `pnpm type-check`                                    | Track A-owned identity fields are now directly proven                                      |
