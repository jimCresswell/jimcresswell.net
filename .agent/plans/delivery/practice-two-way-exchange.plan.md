...
| 2026-09-24 | PR 185, Copilot on the cure tip (thread on `record-generalisation-moves.md:58`) | The transition contract for an estate that kept a register was added by `a359d65c`, and that commit carries no `Practice-Generalisation:` trailer, so the list the rule's query derives from history omits it. Verified. | Rejected as a cure in PR 185: the cure asked for rewrites pushed history, which this estate never does, and the rule's own text says the trailer is written in the commit that makes the change, never added later. The gap stays in history and this row records it: `a359d65c` is a generalisation move (the rule made to work for an estate that kept a register), and the list is completed by hand from these rows. |
| 2026-09-26 | The docs-adr review of `13cf8878` (the register landings pull request), on the closed generalisation register's banner | The banner said every move after the closing carries the trailer but `a359d65c`. A sweep of `ca811fe2..0caa0327` under the rule's globs and the platform adapters finds that the rule's query lists none of the later moves: `4fcf3cab` (the PDR-009 adapter rule, received from the lineage) carries the trailer in a paragraph a blank line parts from the `Co-Authored-By:` block, and git reads trailers from the final paragraph only; `1cf3a1c3` (PDR-009's substance prohibitions stated by their domain) and `8966f554` (the bot-identity core, its host facts moved to their bindings, received from the lineage) carry none. Verified. | Cured in the banner, which names all four; history is never rewritten and a trailer is never added later, so the list is completed by hand from these rows. The other commits in the window are fixes, joint cures and records whose effect is not generality. |
