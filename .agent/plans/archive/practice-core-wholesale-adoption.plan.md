...
Outcome: clean baseline with this plan checked in to the repo, plan directory aligned with the canonical lifecycle convention, dead Cursor-plugin hooks verified absent, and current gates passing before any structural change.
...
Acceptance: at Phase 1 execution time this plan exists at `.agent/plans/active/practice-core-wholesale-adoption.plan.md`; on completion the record archives to `.agent/plans/archive/practice-core-wholesale-adoption.plan.md`; `.agent/plans/active/README.md` points to the primary active plan; plan-directory lifecycle lanes match the canonical convention (`active/`, `current/`, `future/`, `archive/` semantics); `.cursor/hooks/` cleared of dead `continual-learning` state; `git status` clean; `pnpm check` and `pnpm test:e2e` green.

Tasks:

- Confirm this plan initially lives at `.agent/plans/active/practice-core-wholesale-adoption.plan.md` and archive it to `.agent/plans/archive/practice-core-wholesale-adoption.plan.md` on completion.
...
- Verify the dead Cursor-plugin hook surface is already absent. Record that `.cursor/hooks/` and `.cursor/hooks/state/` are gone, and leave the remaining terminology cleanup for Phase 9. The Practice has its own learning loop (napkin → distilled → `consolidate-docs`); the dead `continual-learning` skill from an uninstalled Cursor plugin is fully replaced and must not regain an implementation surface.
...
- Mark plan complete; archive into `.agent/plans/archive/`.

## Notes

- The dead `continual-learning` Cursor-plugin skill is fully replaced by the Practice's own learning loop (napkin → distilled → `consolidate-docs`). Phase 1 records that the orphaned hook implementation is already absent; Phase 9 sweeps any remaining textual references in `AGENTS.md` and `.agent/skills/distillation/SKILL.md`.
...
## Pre-state snapshot — 2026-04-19

This appendix records the verified starting state before the structural
adoption edits landed.

### Baseline proof

- Working tree was clean on `main`.
- Read-only gate sequence passed: `pnpm format:check`, `pnpm markdownlint:check`, `pnpm lint:check`, `pnpm typecheck`, `pnpm test`, `pnpm knip`, `pnpm gitleaks`, `pnpm portability:check`.
- `pnpm test:e2e` passed against a production build.

### Plan and prompt estate

- `active/` incorrectly held two plans: this adoption plan and the PKG source-of-truth design plan.
- Lifecycle lanes were still `active/`, `current/`, `complete/`, `icebox/`, `research/`, with a loose root-level `graph-metaplan.plan.md`.
- Prompt set at start: `dev-tooling-hygiene.prompt.md`, `linkedin-content-preparation.prompt.md`, `personal-knowledge-graph-track-a-cv-metadata-proof.prompt.md`, `personal-knowledge-graph-track-a-external-validation.prompt.md`, `personal-knowledge-graph-track-b-source-of-truth-design.prompt.md`, `session-continuation.prompt.md`, `tilt-retirement.prompt.md`.

### Directives, rules, commands, skills, and reviewers

- Directives at start: `AGENT.md`, `rules.md`, `testing-strategy.md`, `editorial-guidance.md`, `metacognition.md`, `privacy.md`, `secops.md`.
- Canonical rule set at start: `code-quality.md`, `invoke-reviewers.md`, `napkin-always-on.md`, `read-practice.md`, `tdd.md`, `type-safety.md`.
- Canonical commands at start: `jc-commit.md`, `jc-consolidate-docs.md`, `jc-editor.md`, `jc-gates.md`, `jc-plan.md`, `jc-start-right.md`.
- Canonical skills at start: `author-skills`, `deslop`, `distillation`, `editorial-voice`, `napkin`, `package-deps-up-to-date`, `pkg`, `project-spec-creation`, `quality-gates`, `start-right`.
- Canonical sub-agents at start: `code-reviewer`, `editor`, `pkg-reviewer`, `test-reviewer`, `type-reviewer`.

### Platform surfaces at start

- Cursor adapters at start: 5 reviewer adapters, 6 command adapters, 6 rule triggers, and 10 skill adapters.
- Claude adapter estate was absent aside from the root `CLAUDE.md` entry point.
- Codex adapters at start: `code-reviewer`, `editor`, `pkg-reviewer`, `test-reviewer`, `type-reviewer` registered under `.codex/config.toml`.
- Portable discovery layer at start: 10 canonical skill adapters plus `jc-commit`, `jc-consolidate-docs`, `jc-editor`, `jc-gates`, `jc-plan`, `jc-start-right`.
- GitHub surface at start: `copilot-instructions.md` and `dependabot.yml`; no CODEOWNERS, no PR template, no ISSUE_TEMPLATE, no workflows, no reviewer wrappers.

### Scripts and supporting context at start

- Validator/scripts surface at start: `generate-pdf.ts`, `validate-portability.mjs`, `validate-practice-fitness.mjs`.
- Practice-context incoming held 12 files and outgoing held 6 files.
- The dead Cursor-plugin `continual-learning` hook implementation was already absent; `.cursor/hooks/` and `.cursor/hooks/state/` did not exist.

## Completion record — 2026-04-19

### Phase close-out

- Phase 1 completed: recorded the pre-state snapshot, standardised plan lanes to `active/`, `current/`, `future/`, and `archive/`, created `.agent/plans/README.md`, moved the loose graph metaplan into `archive/`, and verified the dead Cursor-plugin hook surface was already absent.
- Phase 2 completed: replaced `.agent/practice-core/` wholesale from the incoming payload, removed the staging copy, and retained the full integration-first surface.
- Phase 3 completed: renamed `rules.md` to `principles.md` and updated the live repo-wide references.
- Phase 4 completed: restructured prompts into canonical subdirectories, archived completed prompts, and reduced active prompts to `start-right` skill references.
- Phase 5 completed: adopted the granular canonical rule set with full Cursor and Claude activation wrappers.
- Phase 6 completed: expanded the reviewer roster to 17 templates, including `mcp-reviewer`, `subagent-architect`, and the four architecture personae, with Cursor, Claude, Codex, and GitHub adapters.
- Phase 7 completed: split `start-right`, added `patterns`, added the missing reviewer-domain skills, and completed the cross-platform discovery layer.
- Phase 8 completed: aligned `package.json`, hooks, docs, and commands to the PDR-008 gate naming contract.
- Phase 9 completed: rewrote `consolidate-docs` to the new three-destination shape and replaced the dead plugin's learning-loop wording with the Practice learning loop.
- Phase 10 completed: created the missing vital surfaces (`practice-core/patterns/`, `memory/patterns/`, `docs/explorations/`, `.agent/hooks/`) and recorded deliberate omissions in the matrix.
- Phase 11 completed: finished the GitHub/Copilot adapter surface, reviewer wrappers, templates, and required workflows for `check`, validators, and `test:e2e`.
- Phase 12 completed: added the portability, sub-agent, vital-surface, practice-fitness, and fitness-vocabulary validators with helper splits and unit-test companions.
- Phase 13 completed: marked ADR-012, ADR-015, and ADR-018 superseded by their PDRs and added PDR-025 for blocking visual proof on rendering-risk work.
- Phase 14 completed: narrowed `practice-context/incoming/` and `outgoing/` to truthful ephemeral surfaces and graduated durable substance into PDRs, patterns, and reference docs.
- Phase 15 completed: regenerated `.agent/practice-index.md`, reconciled `AGENT.md`, and anchored PKG, editorial, personal-identity, CV-as-product, and contributed-validator clusters.
- Phase 16 completed locally: blocking validators, `pnpm check`, `pnpm check:ci`, and `pnpm test:e2e` all passed on the migrated tree.

### Rule mapping table

| Legacy shape          | Canonical result                                                                                 | Action        |
| --------------------- | ------------------------------------------------------------------------------------------------ | ------------- |
| `read-practice.md`    | `follow-the-practice.md`                                                                         | Renamed       |
| `napkin-always-on.md` | `napkin-always-active.md`                                                                        | Renamed       |
| `invoke-reviewers.md` | `invoke-code-reviewers.md` + per-specialist invoke rules                                         | Split         |
| `code-quality.md`     | `apply-architectural-principles.md` + `lint-after-edit.md`                                       | Split/refined |
| `tdd.md`              | `tdd-for-refactoring.md` + `no-skipped-tests.md` + `no-global-state-in-tests.md`                 | Split         |
| `type-safety.md`      | `no-type-shortcuts.md` + `strict-validation-at-boundary.md`                                      | Split         |
| _(missing)_           | `read-agent-md.md`, `tsdoc-and-documentation-hygiene.md`, `subagent-practice-core-protection.md` | Added         |

### Upstream Core review note

- Contradiction surfaced: none.
- Extensions adopted locally without breaking canon: PKG cluster, editorial governance, personal-identity defensives, CV-as-product ADRs, and the explicit unsupported-platform notes in the surface matrix.
- Reusable Practice substance promoted locally: PDR-025 (`rendering-risk-needs-blocking-visual-proof`) and the five-platform adapter + validator split, both recorded as outbound candidates in `practice-context/outgoing/next-outbound-pass.md`.
- Supersession work completed: ADR-012 → PDR-011, ADR-015 → PDR-009, ADR-018 → PDR-007.
- Remaining drift intentionally deferred: advisory fitness overages in `AGENT.md`, `practice.md`, `practice-bootstrap.md`, and `practice-lineage.md`. Per the integration-first principle, those are carried into the post-integration reconciliation session rather than trimmed here.

### Verification evidence

#### Bootstrap Checklist

1. Practice Core package complete: `practice.md`, `practice-lineage.md`, `practice-bootstrap.md`, `practice-verification.md`, `README.md`, `index.md`, `CHANGELOG.md`, `provenance.yml`, `decision-records/`, `patterns/`, and `incoming/.gitkeep` present.
2. `.agent/practice-index.md` regenerated and linked against the migrated estate.
3. `AGENT.md` and `AGENTS.md` point back into the canonical directive chain.
4. Canonical command, rule, skill, and reviewer references were validated by `pnpm validate-portability`.
5. Reviewer reading requirements were validated by the sub-agent surface walk and spot-checked during reviewer/skill authoring.
6. `AGENTS.md` points to `AGENT.md`, which points to `principles.md` and `testing-strategy.md`.
7. `start-right-quick` and `start-right-thorough` exist canonically with adapters on Cursor, Claude, and the cross-platform discovery layer.
8. The napkin rule points to the napkin skill and the live `napkin.md`.
9. Canonical gate names are installed in `package.json`, with `check` aliased to `check:fix`.
10. The repo builds under the Playwright production-build flow used by `pnpm test:e2e`.
11. PDR-009 portability passes across Cursor, Claude, Codex, GitHub Copilot, and the cross-platform discovery layer.
12. Cohesion was re-audited through the migrated docs, commands, skills, prompts, and practice index.
13. PDR-024 vital surfaces pass under `pnpm validate-vital-surfaces`.

#### Post-Installation Health Check

1. Metacognition directive still carries named layers and the grounding anchor.
2. Escape-hatch scan kept the mandatory wording in rules and principles.
3. Link-resolution-sensitive surfaces were exercised by portability and vital-surface validation.
4. Rule adapters exist on the configured platforms (`pnpm validate-portability`).
5. The metacognition command and directive remain installed and reachable from the command surface.
6. Continuity hosts exist: `go`, `session-handoff`, `active/README.md`, `roadmap.md`, and the prompt handoff surfaces.
7. `practice-index.md` now reflects the live estate, including the expanded reviewer and skill roster.
8. Runtime smokes passed through the validator suite plus `pnpm check`, `pnpm check:ci`, and `pnpm test:e2e`.

#### Claimed / Installed / Activated audit

- Claimed: `AGENT.md`, `practice-index.md`, `CLAUDE.md`, `copilot-instructions.md`, and the surface matrix now advertise the same five-platform estate.
- Installed: canonical commands, rules, reviewer templates, skills, hooks, patterns, workflows, and validators are present on disk.
- Activated: Cursor and Claude rule adapters exist, Codex reviewers are registered in `.codex/config.toml`, GitHub reviewer wrappers/workflows exist, and the blocking validators execute successfully against the installed estate.

#### Fresh-checkout acceptance

1. The entry-point chain (`AGENTS.md` → `AGENT.md` → Core/directives) is intact.
2. `go` and `session-handoff` exist as canonical commands with platform adapters.
3. The planning scaffold exists (`.agent/plans/README.md`, lane directories, active-plan README).
4. Hook policy surfaces exist and are documented even though no live hook runtime is installed.
5. `pnpm validate-portability` passes on the local checkout.
6. The bridge surfaces (`practice-index.md`, matrix, roadmap, active-plan README) describe the installed estate truthfully.

### Gate results

- `pnpm validate-vital-surfaces` — pass
- `pnpm validate-portability` — pass (`commands: 11, skills: 21, rules: 29, reviewers: 17`)
- `pnpm validate-subagents` — pass (`templates: 17`)
- `pnpm validate-practice-fitness:informational` — advisory warnings/violations recorded for post-integration reconciliation only
- `pnpm validate-fitness-vocabulary` — pass
- `pnpm check` — pass
- `pnpm check:ci` — pass
- `pnpm test:e2e` — pass (`57` tests)
- GitHub Actions workflow files are installed and pinned, but an actual remote PR-shaped run was not triggered in this local-only session.
