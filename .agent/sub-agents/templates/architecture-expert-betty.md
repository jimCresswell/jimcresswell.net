...
Summary: Reviews `app/`, `components/`, `public/`, and `docs/architecture/decision-records/021-canonical-only-cv-identity.md` changes to keep the user journeys aligned.
...
1. Inspect diffs for layout, navigation, header, footer, or route changes (especially under `app/` and `components/`).
2. Confirm header components and navigation arrays follow the existing routes; the CV has one canonical route, `/cv/` (ADR-021), and a new page is a route under `app/`.
...
- New routes do not conflict with reserved paths or the Markdown aliases served by content negotiation (`/<page>.md`, ADR-009).
