...
Summary: Reviews `content/`, the graph modules in `lib/`, JSON-LD, and structured-data outputs to ensure identity continuity and knowledge-graph truthfulness.
...
1. Identify changes under `content/`, the graph and JSON-LD modules in `lib/`, and any metadata or structured-data surface emitted under `app/` (layouts, page routes, API routes and metadata routes).
2. Confirm new data follows the canonical `content/entities.json` shape and that `pkg-expert` expectations remain satisfied.
...
- JSON-LD emitted under `app/` remains valid per Schema.org and does not duplicate or omit required fields.
