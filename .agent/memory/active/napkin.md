...
- **Owner ask:** reflect on jimcresswell.net, OCE and castr, report, stop. Read-only. One
  registration event (f6ab08fd); no watcher, heartbeat or claim at n=1.
- **Measured, not surveyed.** Set-diffs by `ls` and `comm` over PDRs, rules, skills, templates,
  ...
  against the ancestor pins (OCE `e477e62f7..HEAD`, here `55649a2..origin/main`).
...
- **Join ceremony run on OCE (09:18Z to 09:21Z):** governance read, home declared, identity
  derived with OCE's CLI (same name), registration b1c30d15, watcher armed and asserted,
  adoption 821a3b59, ARC announce 33ab743c, channel file opened, native ping answered.
  Zephyr's n=2 team-start names this seat. Mirror here: 62087b34.
- **Lineage-bound text in `set-up-worktree-lane`:** step 2 expects a bot committer email;
  ...
  committer (origin/main commits 1f30331b, 77d52beb, a9e5f372) with the bot merging. Not
...
- **Ancestor check (09:30Z):** Zephyr named the #135 fold `69a537717` as the ancestor; git
  shows it is an ancestor of the node's pin `e477e62f7` (#138), whose intervening commits this
  estate took at transplant time (register row b57e735). The pin stands; evidence sent.
...
- **Lane `feat/exchange-delta`, commit a1a74d04 (10:55Z):** the delta script, pins, four lists
...
- **Zephyr's dispositions (09:40Z on their clock; read 13:00Z):** L6/L9/J5 are one pr-watch
  ...
  the channel.
- **Register lane (`feat/exchange-register`, cut from the delta lane, committed 09:51Z as e1a2d2ce):** 56
...
- **Correction (14:16Z):** the rendered shared comms log is git-ignored like the events; the render cured the local substrate check and nothing was committed for it. The fold push landed the merge at 325a17c1 on the second attempt. Lesson stands as: `comms append` then `comms render`, in the same turn.
