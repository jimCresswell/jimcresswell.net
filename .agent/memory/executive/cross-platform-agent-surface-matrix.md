...
| -------------- | ------------------- | ------------------------------------------------------ | ------------------------------------------------- | ---------------------------------------------------- | -------------------------------------------------------- | ---------------------- |
| **Skills**     | `.agents/skills/`   | `.claude/skills/`                                      | `.agents/skills/`                                 | `.agents/skills/` exists; acceptance target above    | `.agents/skills/jc-*/` loaded as native Codex skills     | `.agents/skills/`      |
| **Commands**   | retired; workflows use `.agents/skills/` | retired; workflows use `.claude/skills/` | none wired; workflows use `.agents/skills/` | no separate command projection | built-in slash commands; repo workflows use skills | repo workflows use `.agents/skills/jc-*/` |
...
| **Sub-agents** | `.cursor/agents/`   | `.claude/agents/`                                      | `.gemini/agents/` (generated by `pnpm portability:fix`) | native custom agents documented; repo target unwired | `.codex/config.toml` → `.codex/agents/*.toml`             | unsupported            |
...
Claude Code also runs a never-blocking `PreCompact` observer
(`agent-tools/src/bin/claude-pre-compact-observe-hook.ts`, run from TypeScript
source). The contract it observed at its first real compaction is recorded
here because the harness documentation disagreed with itself; it was seen on
