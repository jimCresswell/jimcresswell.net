...
| [LinkedIn Update](current/linkedin-update.plan.md)                                       | Owner-active | Private editorial routing and safety only. Granular state remains behind the private boundary; LinkedIn remains distinct from the CV and graph.                                     | Follow the private plan and handoff only at Jim's direction.                          |
| Tilt Retirement (`archive/tilt-retirement.plan.md`, archived)                                       | Complete     | PR #36 merged the canonical-only CV, removed the live audience-tilt surface, and preserved its public rationale under ADR-021.                                                      | Reopen only through the preserved re-entry route and a new owner-approved plan.       |
...
| [Workspace Architecture Roadmap](current/workspace-architecture-roadmap.plan.md)         | In progress  | Accepted Sequence R plan family. The three-PR repair/publication arc is closed; no workspace manifest exists, and every package hypothesis keeps an internal-module losing outcome. | Follow the Visual child start and record its extraction disposition.                  |
| [Visual Regression Workspace](current/visual-regression-workspace.plan.md)               | In progress  | The validated configuration boundary is merged without workspace discovery or source movement; Tasks 3–5 remain one atomic slice.                                                   | Resolve the allowance-policy API question and record dated PASS/FAIL before any move. |
...
**The graph reset record is** `archive/graph-metaplan.plan.md`.
**Historical PKG references live in** `archive/personal-knowledge-graph-phase-model.plan.md` **and** [research/personal-knowledge-graph-design-notes.md](research/personal-knowledge-graph-design-notes.md).
...
├── LinkedIn Update (owner-active behind the private boundary; no public repo task)
...
workspace family are parallel-runnable around it; LinkedIn remains an
owner-active private editorial lane and tilt retirement is complete.
...
- **LinkedIn** is owner-active behind the private editorial boundary and has no
  architectural dependency on either. The public repository carries routing
  constraints only, not granular editorial state.
...
| ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- |
| Graph Metaplan (`archive/graph-metaplan.plan.md`, archived)                                     | The reset that established the truthful baseline, required the value-led reassessment, and handed off to the adopted graph roadmap. |
| Practice Core Wholesale Adoption (`archive/practice-core-wholesale-adoption.plan.md`, archived) | The structural Practice migration: canonical lanes, five-platform adapter estate, validator suite, and workflow contract.           |
| Codex Platform Alignment (`archive/codex-platform-alignment.plan.md`, archived)                 | Real Codex reviewer sub-agents, canonical thin-adapter split, and ADR-015 for the durable Codex model.                              |
| Visual Regression Harness (`archive/visual-regression-harness.plan.md`, archived)               | Review-oriented non-destructive harness, `WORKTREE` support, and the closed PKG proof record.                                       |
| Optional App Relocation (`archive/optional-app-relocation.plan.md`, archived)                   | Sequence R owner decision: retain the root application; reopen only if a clean orchestration root gains explicit value.             |
| Experience Editorial (`archive/experience-editorial.plan.md`, archived)                         | Research rewrite, voice fixes, capabilities consistency (21 items).                                                                 |
| Capabilities Editorial (`archive/capabilities-editorial.plan.md`, archived)                     | Five blended capabilities with concrete anchors.                                                                                    |
| Meta & SEO Content Audit (`archive/meta-seo-content-audit.plan.md`, archived)                   | `meta.summary`, KNOWS_ABOUT (20→34), OCCUPATION, OG descriptions.                                                                   |
| Front Page Content (`archive/front-page-content.plan.md`, archived)                             | Personal narrative front page.                                                                                                      |
