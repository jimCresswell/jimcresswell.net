...
| 3 | `.claude/skills/slack-watcher/SKILL.md` + `talk-to-slack-watcher/SKILL.md` | Claude adapter | thin pointers; descriptions verbatim-synced; drift gate-checked by `skills:check` (pre-push) |
| 4 | `.agents/skills/slack-watcher/SKILL.md` + `talk-to-slack-watcher/SKILL.md` | cross-tool adapter | thin pointers, same descriptions |
| 5 | `.claude/settings.json:79-82` | permissions | `Skill(slack-watcher)`, `Skill(talk-to-slack-watcher)` + `:*` forms pre-approved |
...
| 11 | `.agent/plans-backlog-2026-07/slack-assistants/` (README, roadmap, `current/ask-oisin.plan.md`, `future/ask-lineage.plan.md`) | backlog | adjacent Slack-assistant organ (Ask Oisín/Ask the lineage); shares Slack coupling, does not define Watcher behaviour |
...
- **L1-F6 — the account-synced `oce-slack-watcher` skill is a second,
  ...
  alongside `slack-watcher`, with a near-identical trigger surface
...
- **L1-F7 — inventory-shape finding (already cured in the ratified
  ...
  the lineage's Claude adapter wrappers, and sibling-estate twins; the sweep found
...
Metered-surface bearing: `chrome-session-is-metered` binds the
lineage-account Chrome seat, which the Watcher's Slack-MCP path does not
...
| the lineage's adapter wrappers, both adapter tiers | **KEEP** | Thin, description-synced, gate-checked (`skills:check`) |
