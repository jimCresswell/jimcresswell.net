...
4. **Resolve class A4 and A5 one way**: restore `corpus-analysis` + `workflow-build` (and
   ...
   estate carries, row 5; the `corpus-analysis` and `workflow-build` restore is pull request
   #86, row 3, open at this writing and read on that row; `the-codex-dialogues` stays at the
   pin.)
