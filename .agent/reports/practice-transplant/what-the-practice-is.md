...
- **A4, A5 (`sif` instruments; the corpus workflow).** The node's ratified todo 1 governs:
  ...
  the pin, plus the workflow file under `.agent/state/` the todo names, which has no corpus
  referent at the pin: nothing is restored under that directory by Director ruling, the
  todo's wording raised to the owner as a card) is restored from the pin, scrubbed,
