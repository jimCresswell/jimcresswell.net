---
classification: situational
description: invoke architecture expert betty
trigger: surface:app/ routes, navigation, layout
---

# Invoke Architecture Reviewer Betty

Invoke `architecture-expert-betty` when changes alter routes, layout composition, navigation,
header or footer behaviour, or user-journey architecture. Use it for experience-architecture
review.

See `.agent/sub-agents/templates/architecture-expert-betty.md` for the full reviewer brief.
