...
**Explicitly NOT asking you to change** (I grounded this): test-expert flagged the plain `if (result.ok) { ... }` narrowing as blocking. It's the established repo idiom (39 test files; 0 use the hard-throw alt), and vitest throws on the prior `expect().toBe`, so no real vacuous pass on the success path. Not a commit-3 defect. The repo-wide question — adopt `@engraph/result`'s existing `unwrap()` as the test idiom — is an owner-gated cleanup; I'll log it as a follow-on, out of your scope.
...
- **4b (oak-curriculum-sdk):** `evidence-corpus/tools/eef-explore-evidence-for-context/`
  ...
  (`@engraph/graph-corpus-sdk: workspace:*`) + ADR-123 (Tools 35→36, Prompts 4→5 +
