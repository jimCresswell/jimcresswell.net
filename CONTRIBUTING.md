...
1. **Set up your environment** — follow the [Getting Started](README.md#getting-started) section in the README. You will need Node.js 24, pnpm, gitleaks and shellcheck.
...
These are repo-specific conventions. For general principles (DRY, KISS, YAGNI, SOLID), see [principles.md](.agent/directives/principles.md).
...
- **Type safety** — no `as`, `any`, or `!`. Label type imports (`import type`). Full details in [principles.md](.agent/directives/principles.md).
...
| `lint:shell` cannot run the pinned shellcheck | Run `.agent/setup/install-shellcheck.sh <bin-dir>` with a directory that comes first on your PATH; it installs the pinned version on macOS and Linux.                                                                     |
