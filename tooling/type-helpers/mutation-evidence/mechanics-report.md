...
```text
This session is isolated in the worktree ~/code/oak/jimcresswell.net-worktrees/typescript-estate-review-019fc3, but this command's working directory resolved to the shared checkout (~/code/oak/jimcresswell.net). Refusing to run it there — a worktree-isolated session's commands must run inside its worktree.
```
...
/**
 * Stryker mutation-testing config for `@engraph/type-helpers` — the
