 ...
 */
describe('@engraph/eslint-plugin-standards strict config: TS-suppression directives', () => {
  ...
  it('does not flag @ts-expect-error with a substantive description under any rule', () => {
    ...
    expect(ruleIds).not.toContain('@typescript-eslint/ban-ts-comment');
    expect(ruleIds).not.toContain('@engraph/no-eslint-disable');
  });
  ...
  it('continues to ban @ts-ignore unconditionally via the custom no-eslint-disable rule', () => {
    ...
    expect(ruleIds).toContain('@engraph/no-eslint-disable');
  });
...
/**
 * Behavioural tests guaranteeing `@engraph/no-real-io-in-tests` fires
...
describe('@engraph/eslint-plugin-standards strict config: no-real-io-in-tests activation', () => {
  it('reports @engraph/no-real-io-in-tests for static fs imports in *.test.ts', () => {
    ...
    expect(ruleIds).toContain('@engraph/no-real-io-in-tests');
  });

  it('reports @engraph/no-real-io-in-tests for process.env access in *.test.ts', () => {
  ...
  it('does not report @engraph/no-real-io-in-tests on non-test files', () => {
    const code = [
      "import { readFileSync } from 'node:fs';",
      "const contents = readFileSync('whatever', 'utf8');",
      'export { contents };',
    ...
    expect(ruleIds).not.toContain('@engraph/no-real-io-in-tests');
  });

  it('does not report @engraph/no-real-io-in-tests on structurally allowlisted test-helpers paths', () => {
    const code = [
      "import { readFileSync } from 'node:fs';",
      "const contents = readFileSync('whatever', 'utf8');",
      'export { contents };',
