...
const fakeContent = {
  ...
  prior_roles: [{ title: "Foundation A", description: ["Description paragraph."] }],
...
describe("CVLayout", () => {
  ...
  it("renders prior roles section with heading and prose", () => {
    render(<CVLayout content={fakeContent} positioning={<p>Positioning text</p>} />);

    const section = screen.getByRole("region", { name: "Before Oak" });
    expect(section).toBeInTheDocument();
    expect(
  ...
  it("renders markdown links in capabilities via RichText", () => {
    const contentWithLinks = {
      ...fakeContent,
      capabilities: ["Built [this site](https://example.com) from scratch"],
    };
    render(<CVLayout content={contentWithLinks} positioning={<p>Positioning text</p>} />);

    const section = screen.getByRole("region", { name: "Capabilities" });
    const link = within(section).getByRole("link", { name: "this site" });
  ...
  it("has correct aria-labelledby attributes on all sections", () => {
    ...
    expect(screen.getByRole("region", { name: "Before Oak" })).toHaveAttribute(
      "aria-labelledby",
      "before-oak-heading"
    );
