...
describe('clearGeneratedAdapters (in-memory seam)', () => {
  ...
  const surfaces = new Map<string, readonly string[]>([
    ['/repo/.claude/skills', ['jc-commit', 'skill-creator']],
    ['/repo/.agents/skills', ['jc-commit', 'clerk', 'skill-creator']],
  ]);
  const stubs = new Map<string, string>([
    ['/repo/.claude/skills/jc-commit/SKILL.md', OURS],
    ['/repo/.claude/skills/skill-creator/SKILL.md', FOREIGN],
    ['/repo/.agents/skills/jc-commit/SKILL.md', OURS],
  ...
  it('removes exactly the directories whose stub carries the class marker, and reports them — membership by content, never by name', async () => {
    ...
    expect(reported).toEqual(
      new Set(['/repo/.claude/skills/jc-commit', '/repo/.agents/skills/jc-commit']),
    );
  ...
  it('preserves and returns the partial teardown when a mid-phase removal fails — the destructive path stays observable', async () => {
    ...
    expect(result.kind).toBe('error');
    expect(result.removed?.map(posixPath)).toEqual(['/repo/.claude/skills/jc-a']);
    expect(removedReal.map(posixPath)).toEqual(['/repo/.claude/skills/jc-a']);
  });
});
...
describe('clearGeneratedAdapters over a real filesystem (the destructive path)', () => {
  it('removes a marker-carrying directory and leaves a foreign one, reporting the removal', async () => {
    ...
    writeRepoFile(root, '.claude/skills/jc-commit/SKILL.md', OURS);
    writeRepoFile(root, '.claude/skills/clerk/SKILL.md', FOREIGN);

    const result = await clearGeneratedAdapters(root);

    expect(result).toEqual({ kind: 'ok', removed: [join(root, '.claude/skills/jc-commit')] });
    expect(repoPathExists(root, '.claude/skills/jc-commit')).toBe(false);
  ...
  it('refuses a symlinked surface ROOT: nothing outside the repo is read or removed', async () => {
    ...
    writeRepoFile(outside, 'skills/jc-victim/SKILL.md', OURS);
    writeRepoFile(outside, 'skills/jc-victim/scripts/important.sh', 'echo keep\n');
    ...
    expect(result.kind).toBe('error');
    expect(repoPathExists(outside, 'skills/jc-victim/SKILL.md')).toBe(true);
    expect(readRepoBytes(outside, 'skills/jc-victim/scripts/important.sh')).toEqual(
  ...
  it('refuses a symlinked surface ANCESTOR with a real skills dir inside it: the external tree is untouched', async () => {
    const root = sandboxRepo();
    const outside = sandboxRepo();
    writeRepoFile(outside, 'dotclaude/skills/jc-victim/SKILL.md', OURS);
    ...
    expect(result.kind).toBe('error');
    expect(repoPathExists(outside, 'dotclaude/skills/jc-victim/SKILL.md')).toBe(true);
  });
  ...
  it('guards BOTH roots before removing anything: a hostile second root leaves the first root intact (no partial pass)', async () => {
    ...
    expect(repoPathExists(root, '.claude/skills/jc-commit/SKILL.md')).toBe(true);
  });
});
