...
describe('clearFirst preflights emission refusals before the destructive clear', () => {
  it('refuses to clear when a canonical would refuse at emit (a committed symlinked references/): the refusing skill’s own projection is NOT lost', async () => {
    ...
    writeRepoFile(
      root,
      '.claude/skills/jc-deploy/SKILL.md',
      projectionStub('deploy/SKILL-CANONICAL.md'),
    );
    writeRepoFile(
      root,
      '.agents/skills/jc-deploy/SKILL.md',
      projectionStub('deploy/SKILL-CANONICAL.md'),
    );

    const outcome = await generateAdapters({ repoRoot: root, prefix: 'jc-', clearFirst: true });
    ...
    expect(repoPathExists(root, '.claude/skills/jc-deploy/SKILL.md')).toBe(true);
    expect(repoPathExists(root, '.agents/skills/jc-deploy/SKILL.md')).toBe(true);
  ...
  it('still clears and regenerates when every canonical CAN emit: no false refusal from the preflight', async () => {
    ...
    writeRepoFile(
      root,
      '.claude/skills/jc-stale/SKILL.md',
      projectionStub('commit/SKILL-CANONICAL.md'),
    );
    ...
    expect(outcome.cleared).toContain(join(root, '.claude/skills/jc-stale'));
    expect(repoPathExists(root, '.claude/skills/jc-stale/SKILL.md')).toBe(false);
    expect(repoPathExists(root, '.claude/skills/jc-commit/SKILL.md')).toBe(true);
    expect(repoPathExists(root, '.claude/skills/jc-deploy/SKILL.md')).toBe(true);
  });
  ...
  it('refuses to clear when a canonical’s current-prefix target is occupied by a foreign entry: the skill’s old-prefix projection is NOT lost', async () => {
    ...
    // emit cannot rebuild deploy at jc-deploy — a lost projection with no
    // canonical-side refusal to catch it (review round 4, 2026-08-12).
    writeRepoFile(root, '.claude/skills/jc-deploy/SKILL.md', 'a foreign, non-marker stub\n');

    const outcome = await generateAdapters({ repoRoot: root, prefix: 'jc-', clearFirst: true });

    expect(outcome.refused.some((entry) => entry.includes('jc-deploy'))).toBe(true);
    expect(outcome.cleared).toEqual([]);
