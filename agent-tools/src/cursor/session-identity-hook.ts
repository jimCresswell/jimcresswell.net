...
export interface CursorSessionIdentityHookEnvironment {
  ...
  readonly PRACTICE_SKIP_COMPOSER_SESSION_MIRROR?: string;
...
export function cursorSessionIdentityHookEnvironmentFromProcessEnv(
  ...
  return {
    ...
    ...(env.PRACTICE_SKIP_COMPOSER_SESSION_MIRROR === undefined
      ? {}
      : { PRACTICE_SKIP_COMPOSER_SESSION_MIRROR: env.PRACTICE_SKIP_COMPOSER_SESSION_MIRROR }),
...
export function planCursorSessionIdentityHook(
  ...
  const tabHint = `Engraph · ${displayName}`;
  const output = {
    env: {
...
function shouldSkipMirror(environment: CursorSessionIdentityHookEnvironment): boolean {
  return environment.PRACTICE_SKIP_COMPOSER_SESSION_MIRROR?.trim() === '1';
}
