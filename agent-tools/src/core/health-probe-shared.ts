...
/**
 * Stable, quote- and expansion-free substring uniquely identifying the
 * PreToolUse policy dispatcher in `.claude/settings.json` and the surface
 * matrix. Used for wiring assertions because the full command embeds
 * `${CLAUDE_PROJECT_DIR}` and JSON-escaped quotes that a literal match would
