 ...
 * runner's own settings. The probe releases the port before the server binds
 * it, so a taker in between is possible: `reuseExistingServer: false` then
 * fails the run loudly instead of proving a server this run did not start.
 *
